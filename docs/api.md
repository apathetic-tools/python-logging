...
## Quick Reference: `apathetic_logging.pytest_helpers` Test Fixtures and Utilities

For isolated logging in test suites. See [testing-guide.md](/testing-guide/) for detailed examples.

| Fixture/Class | Summary |
|-------|---------|
| [`isolated_logging`](#isolated_logging-fixture) | Complete logging state isolation for tests |
| [`logging_test_level`](#logging_test_level-fixture) | Prevent root logger downgrades during tests (TEST level) |
| [`logging_level_testing`](#logging_level_testing-fixture) | Test that your app correctly changes log levels |
| [`apathetic_logger`](#apathetic_logger-fixture) | Fresh logger with unique name and TEST level |
| [`StreamCapture`](#streamcapture-context-manager) | Capture log records to count message occurrences |
| [`LoggingIsolation`](#loggingisolation-class) | Helper class for managing logger state in tests |
...
## `apathetic_logging.pytest_helpers` Reference

Complete reference for pytest fixtures and testing utilities provided by Apathetic Python Logger.

For practical examples and best practices, see [testing-guide.md](/testing-guide/).

### isolated_logging fixture

```python
@pytest.fixture
def isolated_logging() -> LoggingIsolation:
```

Complete logging state isolation for tests. This fixture provides a `LoggingIsolation` helper that saves the entire logging system state before your test and restores it afterwards. This includes all logger handlers, levels, propagate flags, and internal caches.

**Returns:**
- `LoggingIsolation`: Helper object for managing logging state in your test
...
| `getLogger(name)` | Get or create a named logger |
| `captureStreams()` | Create a context manager to capture log records (returns `StreamCapture`) |
| `saveState()` | Manually save current logging state |
| `restoreState()` | Manually restore a saved logging state |

**Usage:**

```python
def test_logging_behavior(isolated_logging):
    """Complete logging state is isolated for this test."""
    root = isolated_logging.getRootLogger()
    child = isolated_logging.getLogger("myapp.module")

    root.setLevel(logging.DEBUG)
    child.setLevel(logging.INFO)

    # Your test code here
    child.info("This message appears once")

    # State is automatically restored after the test
```

**When to Use:**
- When you need to modify logger levels, handlers, or propagate settings
- When you want to test isolated logger behavior without affecting other tests
- When testing your application's logging setup during startup
- For integration tests involving multiple loggers

**Benefits:**
- Works in serial mode, parallel mode (xdist), package mode, stitched mode, and zipapp mode
- Completely isolates logging state—no side effects between tests
- Resets internal caches to prevent state pollution

---

### logging_test_level fixture

```python
@pytest.fixture
def logging_test_level() -> None:
```

Prevent root logger downgrades during tests. This fixture ensures the root logger stays at TEST level (or more verbose) during your test, preventing messages from being silenced.

**Returns:**
- `None` (fixture just sets up the environment)

**Use Case:**
- When you're testing a logger that inherits its level from root (default behavior)
- When you want to ensure debug/test messages aren't filtered out by a silent root logger
- When you want to test output that depends on log level being high enough

**Automatic Behavior:**
- Sets root logger to TEST level before test runs
- Restores original level after test completes
- Prevents "your test appears to work, but only because of the root level" surprises

**Example:**

```python
def test_my_logger_debug_output(logging_test_level):
    """Debug output appears because TEST level is high enough."""
    logger = logging.getLogger("myapp")
    logger.setLevel(logging.DEBUG)

    # Works even if something set root to a lower level
    logger.debug("This appears")
```

---

### logging_level_testing fixture

```python
@pytest.fixture
def logging_level_testing() -> logging_level_testing:
```

Test that your application correctly changes log levels. This fixture marks your test as specifically testing log level behavior, ensuring the test framework doesn't interfere with level changes.

**Returns:**
- Fixture object (typically used via marker or parametrization)

**Usage as Marker:**

```python
@pytest.mark.logging_level_testing
def test_log_level_change():
    """Test that level changes work correctly."""
    logger = logging.getLogger("myapp")
...
### apathetic_logger fixture

```python
@pytest.fixture
def apathetic_logger(request) -> Logger:
```

Provide a fresh logger with a unique name for each test. Creates an instance of `apathetic_logging.Logger` with a name based on the test module and function, set to TEST level.

**Returns:**
- `Logger`: An Apathetic Python Logger instance ready for testing

**Features:**
- Unique name per test (prevents cross-test logger registry collisions)
- Starts at TEST level (ensures test messages aren't filtered)
- Properly isolated via the `isolated_logging` fixture
- Great for testing custom logger behavior

**Example:**

```python
def test_custom_logger_features(apathetic_logger):
    """Test custom logger methods."""
    logger = apathetic_logger

    # Logger is already at TEST level
    logger.test("This is a test message")
    logger.brief("This is brief")
    logger.detail("This is detail")
```
...
```python
class StreamCapture:
    """Context manager for capturing log records to detect log duplication."""
...
**Why Use StreamCapture Instead of caplog:**
- Works in xdist parallel mode (caplog doesn't)
- Works in stitched mode (caplog doesn't)
- Works in all runtime modes (package, stitched, zipapp)
- Simple and explicit message counting
...
```python
def test_no_duplicate_output(isolated_logging):
    """Verify message appears exactly once."""
    logger = isolated_logging.getLogger("test")
    logger.setLevel(logging.DEBUG)
...
**When to Use:**
- Testing that log messages appear exactly once (no duplication)
- Counting occurrences of specific messages in test output
- Integration tests that verify logging behavior
- Symptom-based tests that detect user-visible logging bugs

**When NOT to Use:**
- When you just need to verify a message was logged (use `caplog` or assert it doesn't error)
- When you need to inspect log record properties (use `caplog.records`)
- When testing in isolation without pytest (implement your own stream capture)

---

### LoggingIsolation class

```python
class LoggingIsolation:
    """Helper class for managing logger state in tests."""
```

Advanced helper for complete control over logging state isolation. Typically accessed via the `isolated_logging` fixture rather than used directly.

**Methods:**

| Method | Description |
|--------|-------------|
| `getRootLogger()` | Get the root logger instance |
| `getLogger(name)` | Get or create a named logger |
| `captureStreams()` | Return a `StreamCapture` context manager |
| `saveState()` | Save the current logging state |
| `restoreState(state)` | Restore a previously saved state |

**Manual State Management Example:**

```python
def test_manual_state_management(isolated_logging):
    """Manually manage logging state at specific points."""
    # Save state at a specific point
    state_before = isolated_logging.saveState()
    ...
    # Later, restore to the saved state
    isolated_logging.restoreState(state_before)
```

**Benefits of LoggingIsolation:**
- Saves and restores: handler list, levels, propagate flags, internal caches
- Prevents test pollution across multiple tests running in parallel
- Works with pytest-xdist and stitched mode
...
```python
def saveLoggingState() -> dict[str, Any]:
```

Save the current state of the entire logging system. This captures all loggers, handlers, levels, and internal state.

**Returns:**
- `dict`: Complete logging state that can be passed to `restoreLoggingState()`

**Use Case:**
- Implementing custom fixtures that need logging isolation
- Saving state at specific points during a test for comparison
...
```python
def restoreLoggingState(state: dict[str, Any]) -> None:
```

Restore the logging system to a previously saved state.

**Parameters:**

| Parameter | Type | Description |
|-----------|------|-------------|
| `state` | dict | State dictionary from `saveLoggingState()` |

**Use Case:**
- Restoring logging state after modifying it during a test
- Implementing complex test scenarios with multiple state changes
...
Remove all handlers from all loggers and reset their levels to default.

**Use Case:**
- Quick cleanup when you don't need full state restoration
- Clearing out handlers between test phases

---

### Integration with isolated_logging

All utilities are accessible through the `isolated_logging` fixture. Here's the recommended pattern:

```python
def test_complex_logging_scenario(isolated_logging):
    """Complex logging test with multiple phases."""

    # Phase 1: Set up initial state
    root = isolated_logging.getRootLogger()
    root.setLevel(logging.WARNING)

    # Phase 2: Test something
    with isolated_logging.captureStreams() as capture:
    ...
    root.setLevel(logging.DEBUG)
    logger = isolated_logging.getLogger("phase2")
