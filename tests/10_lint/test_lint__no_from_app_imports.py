...
- runtime_swap: Tests can run against either package or stitched
  ...
  is available for runtime swapping. `from ... import` breaks this because
  imported items are no longer associated with their module object.
- patch_everywhere: Predictive patching requires module objects to be
  available at the module level. Using `from ... import` breaks this because
  the imported function is no longer associated with its module object.
...
def test_no_app_from_imports() -> None:
    ...
    Custom lint rule implemented as a pytest test because we can't create custom
    ruff rules yet. It ensures all test files use module-level imports required
